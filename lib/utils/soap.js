...
class Soap {
  ...
  createRequest (params) {
    ...
    // soap += '<wsa:Action s:mustUnderstand="1">'
    ...
    // soap += '</wsa:MessageID>'
    soap += '<wsa:ReplyTo>'
    soap += '<wsa:Address>http://www.w3.org/2005/08/addressing/anonymous</wsa:Address>'
    soap += '</wsa:ReplyTo>'
    if (params['username']) {
    ...
    if (params.subscriptionId) {
      ...
      if (address) {
        soap += '<wsa:To s:mustUnderstand="1">'
        soap += address
        soap += '</wsa:To>'
      }

      if (params.subscriptionId) {
        let xml = this.getCustomSubscriptionIdXml(params.subscriptionId)
        if (xml) {
          soap += xml
        }
      }
   ...
   */
  makeRequest (serviceAddress, methodName, soapEnvelope, params) {
    let promise = new Promise((resolve, reject) => {
      ...
      return this.runRequest(serviceAddress, methodName, soapEnvelope)
        ...
        .then(results => {
          // is this 'raw' data?
          if ('raw' in results) {
            Save.saveXml(methodName + '.Response', results.soap)
            resolve(results)
            return
          }
      ...
      if (subscriptionId._) {
        ...
        if (subscriptionId.$) {
          ...
          if (id && tag && url) {
            ...
            xml = '<SubscriptionId s:mustUnderstand="1"' + tags[0] + '="' + url + '">' + id + '</SubscriptionId>'
            console.log(xml)
          }
        }
        return xml
