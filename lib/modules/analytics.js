...
class Analytics {
  ...
  requestWithConfigurationToken (methodName, configurationToken, xml, callback) {
    let promise = new Promise((resolve, reject) => {
      ...
      if (typeof xml !== 'undefined' && xml !== null) {
        if ((errMsg = Util.isInvalidValue(xml, 'string'))) {
      ...
      let soapBody = ''
      soapBody += '<axt:ConfigurationToken>' + configurationToken + '</axt:ConfigurationToken>'
      if (typeof xml !== 'undefined' && xml !== null) {
        soapBody += xml
      }
  ...
  deleteAnalyticsModules (configurationToken, analyticsModuleName, callback) {
    let soapBody = '<axt:AnalyticsModuleName>' + analyticsModuleName + '</axt:AnalyticsModuleName>'
  ...
  getAnalyticsModuleOptions (configurationToken, type, callback) {
    let soapBody = '<axt:Type>' + type + '</axt:Type>'
  ...
  deleteRules (configurationToken, ruleName, callback) {
    let soapBody = '<axt:RuleName>' + ruleName + '</axt:RuleName>'
   ...
   * @param {string} ruleType Reference to an SupportedRule Type returned from Analytics#getSupportedRules.
   * @param {callback=} callback Optional callback, instead of a Promise.
   */
  getRuleOptions (configurationToken, ruleType, callback) {
    let soapBody = '<axt:RuleType>' + ruleType + '</axt:RuleType>'
